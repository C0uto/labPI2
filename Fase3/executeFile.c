...
void tratarAjuda(void) {
    printf("\nComandos:\n  p <origem> <destino> [n] - Move n cartas\n  u             - Undo\n  e             - Estado\n  s             - Gravar\n  r             - Reiniciar\n  h             - Ajuda\n  q             - Sair\n");
}
...
void tratarMover(char *buf, ESTADO *j, RegrasMovAuto rma, RegrasWin rw) {
    int o, d, n = 1;
    if (sscanf(buf, " p %d %d %d", &o, &d, &n) < 2) { printf("Uso: p <origem> <destino> [n]\n"); return; }
    guardarSnapshot(j);
    if (!tentarMover(j, o - 1, d - 1, n, rma, j->rt)) {
        SNAPSHOT *s = j->historico; j->historico = s->prox; libertarSnapshot(s); printf("Movimento invalido!\n");
    } else {
        printf("\n[MOV] Movimento executado: %d -> %d (%d cartas)\n", o, d, n);
        mostrarEstado(j); processarAuto(j, rma, j->rt);
 ...
 * * @param j -> Estado do jogo
 */
void tratarUndo(ESTADO *j) { if (restaurarSnapshot(j)) mostrarEstado(j); }
...
void loopComandos(ESTADO *j, RegrasMovAuto rma, RegrasInit ri, RegrasBaralhos rb, RegrasWin rw, const char *nome) {
    char buf[256]; int continuar = 1, input_ok = 1;
    while (continuar && input_ok) {
        printf("%s> ", nome ? nome : "Paciencia");
        if (!fgets(buf, 256, stdin)) input_ok = 0;
        else {
            buf[strcspn(buf, "\n")] = '\0';
            if (buf[0]) continuar = executarComando(buf, j, rma, ri, rb, rw, nome);
        }
    }
}
...
void prepararAmbiente(ESTADO *j, RegrasBaralhos rb, RegrasMovAuto rma, RegrasWin rw) {
    ...
    j->B = criarBaralho(n_baralhos); j->total_cartas_baralho = 52 * n_baralhos;
